...
const EPI_ROUTES = [
    ...
    { method: "post", path: "/payments/{paymentKey}/transactions", tag: "Payment", contextful: true, summary: "Capture, release or refund", request: "TransactionInitDto", response: "TransactionDto",
      when: "The cashier captures a reservation, releases it, or refunds a completed sale. CommerceOS adds one payment record from the answer. It calls once and does not retry. Treat every call as a new transaction: CommerceOS never retries this call, and two equal partial refunds of one line arrive with the same token and body, so answering the second with the first would refund the customer once instead of twice. Refuse a call for more than the payment has left. A key whose stream did not end in Complete has no payment: answer 404 with an error body." },
