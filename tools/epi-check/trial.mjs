...
// CommerceOS. After each attempt the harness starts the integration, runs epi-check in local mode
// against it (a stand-in CommerceOS that installs and configures it), and hands the report back.
// The agent's own notes on what the documents left unclear are the second output: they name the
// next documentation fix.
//
...
const BASE_PATH = "/epi";
/** The configuration the tool's stand-in saves on the node, which the task names so that /test can check it. */
export const CONFIGURATION = { merchantId: "TRIAL-0001", environment: "TEST" };
...
2. The single payment method has the id \`${METHOD_ID}\` and requires no terminal. Answer an empty
   terminal list.
3. Your configuration form has two fields, \`merchantId\` and \`environment\` (\`TEST\` or \`LIVE\`), and
   \`/test\` checks them. Heads tests with \`merchantId\` \`${CONFIGURATION.merchantId}\` and \`environment\` \`${CONFIGURATION.environment}\`.
4. Follow the test amounts of the tutorial (the cents of the amount select the outcome). Heads
   runs their conformance tool in local mode against your endpoint; that tool is your acceptance
   test. In local mode the tool starts a stand-in CommerceOS and does what an administrator does:
   it installs your integration (the install payload points at the stand-in, which answers the
   token endpoint, the configuration behind the context id and the key-value store), saves the
   configuration above on a node, and reads your methods. Then it runs the scenarios.
...
/** Starts `server.py`, runs epi-check in local mode against it, stops it. Returns the run result and the server log. */
export async function checkAttempt(workspace, outDir) {
    ...
    try {
        ...
        writeFileSync(profile, JSON.stringify({ methodId: METHOD_ID, configuration: CONFIGURATION }, null, 4) + "\n");
        const result = await run({ local: baseUrl, profile, out: join(outDir, "epi-check"), timeout: 10000 });
...
export async function trial(options) {
    ...
    try {
        for (let n = 1; n <= options.attempts && !passed; n++) {
            ...
            const checked = await checkAttempt(workspace, attemptDir);
