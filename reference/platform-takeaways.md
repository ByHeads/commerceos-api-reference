...
- **A scope problem answers `200`, `400` or `404` — never `403`.** Nothing in the API raises a permission error: a scope the token lacks makes its resources absent rather than refused. Several resources exist twice, read-only under `<area>:read` and writable under `<area>:write`, at the same path, and a write that lands on the read-only twin is a `200` that persists nothing. A read the scopes do not reach is a `404`; a write that needed a writable resource they do not reach is a `400`, which is the one that reads like a bad payload. Confirm a write by reading it back, never by its status (see [gotcha 41](common-gotchas.md#41-a-write-under-a-read-only-scope-is-a-silent-200))
