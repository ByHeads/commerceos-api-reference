...
- **Key detection on `@type`, not on the other fields.** Both streamed markers carry `@type`, `error` and `innerError`, but `processedCount` is `application/json`-only and `failedAtIndex` never appears on a streamed marker at all. So `if (last.processedCount !== undefined)` never fires on an NDJSON, CSV or SQL export — and waves a truncated one through as complete. Test `"@type": "mid-stream error"` and read `innerError` for the cause ([field matrix](../features/streaming.md#which-fields-each-form-carries)).
- **`innerError` has no `@type` of its own.** It is the same `{"error": ..., "details": ...}` object on every format, which makes it uniform to read — but the wrapper around it *does* carry an `@type`, and so does a non-streaming error body, so a parser that expects one everywhere is wrong exactly here. Match on `error`/`details`.
