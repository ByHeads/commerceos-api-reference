...
Note there is no `orders:read`: order data is read through `trade-records:read` and `logistics:read`, while `orders.sales:write` and `orders.payments:write` cover the write side. `deliveries:*` and `returns:*` ship in v26.2.1 and later (not in v26.1.x) and are not part of `logistics:*` or `read:api`.
