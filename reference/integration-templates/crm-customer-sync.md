...
>
> On `people` the presence half has a sharper edge than a dropped record. A person with neither a `givenName` nor a `familyName` has no `fullName` at all, and if such a person sorts first the request fails with a `400` reading `Invalid sort key 'fullName': field not found` rather than returning an unsorted or partial page — the sort key is checked against the first record only ([gotcha 39](../common-gotchas.md#39-sorting-on-an-empty-field-is-a-400-not-an-empty-slot)). A name member remains perfectly usable as a *filter* — `~where(fullName>"…")` is fine — which is why the two look interchangeable right up until you try the sort on a dataset with a nameless record in it.
>
