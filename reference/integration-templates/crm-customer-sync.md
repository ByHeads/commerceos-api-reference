...
>
> **The requirements are shared with the `after` token form; the consequence of breaking them is not.** You are writing the `~where(fullName>"<last>")` predicate yourself, so nothing checks it: a run of equal values at a page boundary is skipped and the sync completes normally. The token form refuses to skip — a page whose last item shares its sort value with the next one mints no token and reports `X-Has-More: true`, so the same mistake stops the walk instead of quietly shortening it. That makes uniqueness something you have to establish up front here, rather than something the API will tell you about.
>
...
>
> **A sort value holding `,`, `&`, `?`, `~` or parentheses is safe on the `after` token form** from the release after v26.1.11 — the token is replayed as a value rather than re-read as query syntax, where v26.1.10 and v26.1.11 stopped such a walk at the first name carrying one of those characters and reported it complete ([details](../pagination.md#requirements-and-notes)). In the hand-written predicate above, the value you paste in is part of the URL, so percent-encode it (`%2C`, `%26`, `%3F`, `%7E`) whichever build you are on.
>
> **If you switch to the `after` token form, project with `fields=` rather than `~just(...)`.** The sort value is added to your projection behind the scenes only when the request carries a `fields=` list, which is why such a list can be as narrow as you like alongside a nested sort selector such as `identifiers/key` — `fields=fullName` walks to the end of the collection. With `~just(fullName)` alone the cursor is minted only if the sort value can be read from what you projected, and it cannot: the request returns `X-Has-More: true` with no `X-Cursor-Next` and the walk never starts. Both ways out work, and the cheap one is to add the parameter: `~just(fullName)` with `&fields=fullName` walks the whole collection and returns exactly the items it returned before. Widening to `~just(fullName,identifiers)` completes the walk too, but puts an `identifiers` object on every person you sync. See [Cursor pagination](../pagination.md#requirements-and-notes).
