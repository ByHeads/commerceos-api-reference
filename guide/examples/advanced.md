...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/products~where(status=Active)~where(name=~Premium)~take(10)"

# Filter by nested property
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/trade-orders~where(supplier/name=~Acme)"

# Filter by count
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~where(items~count>5)~take(10)"
```
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/trade-orders/com.myapp.orderId=ORD-001/customer/addresses/main"

# Get nested count
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/product-categories/com.heads.seedID=electronics/members~count"

# Deep with clause
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/trade-orders~with(items~with(product~just(name)))~take(5)"
```
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~just(receiptId:identifiers/receiptID,itemCount:items~count)~take(20)"
```
...
# Nested alias
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/trade-orders~just(orderId:identifiers/key,customerName:customer/name,supplierName:supplier/name)~take(10)"

# Alias with aggregation
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~just(id:identifiers/receiptID,items:items~count,total:totalAmount)"
```
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~map(com.myapp.receipt-summary)~take(5)"
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~map(com.myapp.receipt-bundle)"
```
...
# Bulk create prices
curl -X PUT -u ":banana" "https://example.app.heads.com/api/v1/prices" \
  -H "Content-Type: application/x-ndjson" \
  --data-binary @- << 'EOF'
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~take(10)~map(com.heads.receipt-csv)" \
  -H "Accept: application/vnd.ms-sqlserver.csv"

# Export as bundled ZIP (receipts + items + payments)
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~take(10)~map(com.heads.receipts-zip)" \
...
curl -X GET -u ":banana" "https://example.app.heads.com/api/v1/receipts~take(10)~map(com.myapp.receipt-sql-delete)" \
  -H "Accept: application/sql"
```
