...
| `events` | For `startPayment`: the ordered list of event types. `Wait`, `Cancellable` and `Create` steps are ignored unless listed, so a waiting message, a cancel button or an early payment order during P1 still passes. A stream sends `Create` at most once, and a `Complete` after it carries the same `processorsId`. Where a scenario lists `Cancellable` (P7), the stream must send it. After a `Cancellable`, a `Wait` or `ShowImage` step must follow in every scenario |
